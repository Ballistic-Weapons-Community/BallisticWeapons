...
var float				BlurEndTime;
var float				BlurTime;
var float				BlurFactor;
var bool				bNoFade;
var bool				bIsBlurred;
var bool				bWasDead;
...
simulated event Tick(float DT)
{
	...
	if (!PC.IsDead())
	{
		...
		bWasDead = true;
	if (level.TimeSeconds > BlurEndTime)
	{
		bIsBlurred=false;
		UnBlurIt();
		BlurTime = 0;
...
simulated function UnBlurIt ()
{
//	PC.ConsoleCommand("set ini:Engine.Engine.AudioDevice SoundVolume"@OldVolume);
	if (MBlur != None)
		PC.RemoveCameraEffect(MBlur);
}
