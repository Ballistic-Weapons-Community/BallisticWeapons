class UTComp_Overlay_BW_AMTAM extends Interaction;
...
function PostRender(Canvas Canvas)
{
	...
	if(Misc_Player_UTComp_LDG(ViewportOwner.Actor) != None)
	{
		if(Misc_Player_UTComp_LDG(ViewportOwner.Actor).UTCompPRI != None)
			uPRI = Misc_Player_UTComp_LDG(ViewportOwner.Actor).UTCompPRI;
	...
	if(ViewPortOwner.Actor.Level.TimeSeconds < OnJoinMessageDrawTimeBW)
	{
		...
		{
			CmdText =	class'GameInfo'.Static.GetKeyBindName("ballistic", Misc_Player_UTComp_LDG(ViewportOwner.Actor));
		...
		{
			CmdText =	class'GameInfo'.Static.GetKeyBindName("mymenu", Misc_Player_UTComp_LDG(ViewportOwner.Actor));
	 		if (CmdText ~= "mymenu")
	 		{
	 			PressText = "Type ";
	 			ChangeTextA = " to";
			...
			Canvas.DrawText("UTComp " $ MakeColorCode(class'Hud'.Default.GoldColor) $ "3SPN BW" $ MakeColorCode(InfoTextColor) $ ".");
			Canvas.SetPos(OverlayLocX, OverlayLocY + 3 * (YL + 2.0));
			Canvas.DrawText(PressText $ MakeColorCode(class'Hud'.Default.GoldColor) $ CmdText $ MakeColorCode(InfoTextColor) $ ChangeTextA);
			Canvas.SetPos(OverlayLocX, OverlayLocY + 4 * (YL + 2.0));
			Canvas.DrawText(ChangeTextB);
...
exec function SetCurrentColoredName(int i)
{
	if (i >= 0 && i < class'UTComp_Settings'.default.ColoredName.Length)
	{
		...
		Misc_Player_UTComp_LDG(ViewportOwner.Actor).SetColoredNameOldStyleCustom(,i);		
