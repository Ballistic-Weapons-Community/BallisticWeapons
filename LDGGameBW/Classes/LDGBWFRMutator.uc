class LDGBWFRMutator extends TAM_Mutator
	HideDropDown
	CacheExempt;
...
function bool IsRelevant(Actor Other, out byte bSuperRelevant)
{
	if (LDGBallisticFR(Level.Game) != None)
	{
		...
		{
			LDGBallisticFR(Level.Game).FlagsServer = LDGUserFlagsServer(Other);
			LDGUserFlagsServer(Other).RcvdFor = LDGBallisticFR(Level.Game).ReceivedPlayerFlags;
		}
		else if (Other.IsA('LDGAbstractBalancer'))
		{
			LDGBallisticFR(Level.Game).TeamBalancer = LDGAbstractBalancer(Other);
			
			if (LDGBallisticFRTracked(Level.Game) != None)
				LDGAbstractBalancer(Other).RequestPlayerVariable = LDGBallisticFRTracked(Level.Game).TeamBalancerPlayerVariable;
		}
	}
