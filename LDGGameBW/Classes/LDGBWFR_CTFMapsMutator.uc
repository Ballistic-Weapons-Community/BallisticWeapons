class LDGBWFR_CTFMapsMutator extends TAM_Mutator
	HideDropDown
	CacheExempt;

function bool IsRelevant(Actor Other, out byte bSuperRelevant)
{
	if (LDGBallisticFR_CTFMaps(Level.Game) != None)
	{		
		if (Other.IsA('LDGUserFlagsServer'))
		{
			LDGBallisticFR_CTFMaps(Level.Game).FlagsServer = LDGUserFlagsServer(Other);
			LDGUserFlagsServer(Other).RcvdFor = LDGBallisticFR_CTFMaps(Level.Game).ReceivedPlayerFlags;
		}
		else if (Other.IsA('LDGAbstractBalancer'))
		{
			LDGBallisticFR_CTFMaps(Level.Game).TeamBalancer = LDGAbstractBalancer(Other);
			
			if (LDGBallisticFR_CTFMapsTracked(Level.Game) != None)
				LDGAbstractBalancer(Other).RequestPlayerVariable = LDGBallisticFR_CTFMapsTracked(Level.Game).TeamBalancerPlayerVariable;
		}
	}
