class LDGBallisticAM extends ArenaMaster;
...
function PlayerBecamingSpectator(Misc_Player_UTComp_LDG PC)
{
	PlayerLeaving(PC);
...
//#ifdef BW_FREON
function PlayerBecameActive(Misc_Player_UTComp_LDG PC)
{
...
function PlayerController Login(string Portal, string Options, out string Error)
{
	...
	if (Misc_Player_UTComp_LDG(PC) != None)
	{
		Misc_Player_UTComp_LDG(PC).PlayerBecamingSpectator = PlayerBecamingSpectator;
		Misc_Player_UTComp_LDG(PC).PlayerBecameActive = PlayerBecameActive;
	}
...
defaultproperties
{
     ...
     BWMutators(2)="LDGGameBW.MutUTCompBW_LDG_AMTAM"
     BWMutators(3)="BWInteractions3.MutBWInteractions"
     HintColor=(B=255,G=255,R=255,A=255)
     MapColor=(G=255,R=255,A=255)
     MapListType="LDGGameBW.LDGBallisticDMMapList"
     MutatorClass="LDGGameBW.LDGBWAMMutator"
     GameName="LDG Ballistic ArenaMaster"
