class UTComp_JBInterfaceHud extends JBInterfaceHud;
...
//Wrappers for widget functions, because I made the main HUD template use these functions
function DrawWidgetAsTile(Canvas C, SpriteWidget W)
{
	DrawSpriteWidget(C, W);
}

function DrawNumericWidgetAsTiles(Canvas C, NumericWidget W, DigitSet D)
{
...
{	
	...
	//update the position not to overlap the icon for status
	...
	if (PC != None)
	{
		...
		{
			if (PC.Player.LocalInteractions[i].class == class'UTComp_OverlayB_BW')
			{
				UTComp_Overlayb_BW(PC.Player.LocalInteractions[i]).bHideOverlay = JBGameReplicationInfo(PlayerOwner.GameReplicationInfo).bIsExecuting;
				if (bDrawTimer && PawnOwner != None && PawnOwner.PlayerReplicationInfo != None)
					UTComp_Overlayb_BW(PC.Player.LocalInteractions[i]).MinPosY = 0.135;
				else
					UTComp_Overlayb_BW(PC.Player.LocalInteractions[i]).MinPosY = 0.065;
				break;
			}
		}
	}
...
//#endif
{
	...
	if ( PawnOwner != None )
	{
		if( bShowWeaponInfo && (PawnOwner.Weapon != None) )
		{
			...
			BarYLen = (ResScaleY * 34 * HUDScale) + (3 * MyBarBorder) * 0.870;
...
defaultproperties
{
     ...
     HudBorderVehicleHealth=(WidgetTexture=Texture'LDGGameBW_rc.HUD.new_hud',TextureCoords=(Y1=55,Y2=108),PosX=0.000000,OffsetX=168)
