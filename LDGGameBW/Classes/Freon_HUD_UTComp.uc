class Freon_HUD_UTComp extends Freon_HUD;


var color FrozenNameColor;
//BEGIN_HUD_DECL
...
// draw health dot
            ...
            if(Freon_PawnReplicationInfo(PRI.PawnReplicationInfo) != None && !Freon_PawnReplicationInfo(PRI.PawnReplicationInfo).bFrozen &&
            	class'UTComp_Settings_FREON'.default.bColoredNamesInHUD && uPRI != None && uPRI.ColoredName != "")
            	DrawColoredNameCorrectly(C, uPRI.ColoredName, Len(name));
            else if(Freon_PawnReplicationInfo(PRI.PawnReplicationInfo) != None && Freon_PawnReplicationInfo(PRI.PawnReplicationInfo).bFrozen)
            {
            	C.DrawColor = FrozenNameColor;
            	C.DrawText(name);
            }
            else               
            	C.DrawText(name);
...
{
	if (UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard) != None)
	{
		if (!bShowScoreboard)
		{
			bShowScoreboard = true;
			UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard).bViewingSkill = class'UTComp_Settings_FREON'.default.bViewSkillByDefaultOnEnhancedScoreboard;
			UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard).bShowingNewSkill = false;
		}
		else if (UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard).bShownNewSkill && !UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard).bDeranked && !UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard).bShowingNewSkill)
			UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard).bShowingNewSkill = true;
		else
...
{
	if (bShowScoreboard && UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard) != None)
		UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard).bViewingSkill = !UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard).bViewingSkill;
	else
		Super.ShowStats();
}
...
{
	if (UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard) != None)
		UTComp_ScoreBoard_NEW_SKILL_FREON(Scoreboard).CheckNewSkill();
...
defaultproperties
{
     FrozenNameColor=(B=160,G=160,R=160,A=255)
