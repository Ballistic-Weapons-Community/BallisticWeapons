...
//=============================================================================
class ARFireControl extends Actor;

const MAX_FIRE_SPOTS = 15;
...
simulated function Initialize()
{
	...
	foreach VisibleCollidingActors( class 'Actor', A, DamageRadius, Location )
	{
		if (xPawn(A)!=None)
			IgniteActor(A);
		class'BallisticDamageType'.static.Hurt(A, (1.0-(VSize(A.Location - Location)/DamageRadius)) * BaseDamage, Instigator, A.Location, Normal(A.Location - Location)*500, class'DTARImmolation');
	}
	...
	for(i=0;i<MAX_FIRE_SPOTS;i++)
	{
		...
		T = Trace(HitLoc, HitNorm, End, Start,, vect(3,3,3));
		if (T==None) 
			HitLoc=End;

		GF = Spawn(class'ARGroundFire',self,,HitLoc, rot(0,0,0));
		if (GF!=None)
		{
...
simulated function PostNetReceive()
{
	...
	if (!bClientFiresSpawned)
	{
		for (i=0;i<ArrayCount(GroundFireSpots);i++)
		{
			...
			{
				F = Spawn(class'ARGroundFire',self,,GroundFireSpots[i], rot(0,0,0));
				if (F != None)
...
{
	if (ARGroundFire(A) == None)
		return;
	Fires[Fires.Length] = ARGroundFire(A);
}
//===========================================================================
...
{
	local ARActorBurner PF;

	PF = Spawn(class'ARActorBurner',self, ,A.Location);
