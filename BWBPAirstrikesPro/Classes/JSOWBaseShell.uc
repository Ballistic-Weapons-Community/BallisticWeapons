...
//
//-----------------------------------------------------------
class JSOWBaseShell extends AirstrikeProjectile;
...
{
    local int i;
    local JSOWProjectile SmallShell;
    ...
    for (i=0; i<3; i++)
    {
        SmallShell = spawn(class'JSOWProjectile', self, , Location, Rotation);
		if ( SmallShell != None )
...
{
	if (Other != Instigator && !Other.IsA('JSOWBaseShell'))
	{
