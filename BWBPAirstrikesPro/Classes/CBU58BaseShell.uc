...
//
//-----------------------------------------------------------
class CBU58BaseShell extends AirstrikeProjectile;
...
{
    local int i;
    local CBU58Projectile SmallShell;
    ...
    for (i=0; i<35; i++)
    {
        SmallShell = spawn(class'CBU58Projectile', self, , Location, Rotation);
		if ( SmallShell != None )
			SmallShell.Velocity = Velocity + (VRand() * 250.0);
    }
    Destroy();
}
...
{
	if (Other != Instigator && !Other.IsA('CBU72BaseShell'))
	{
...
defaultproperties
{
     ...
     MyDamageType=Class'BWBPAirstrikesPro.DamTypeCBU58Bomb'
