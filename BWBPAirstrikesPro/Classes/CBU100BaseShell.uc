...
//
//-----------------------------------------------------------
class CBU100BaseShell extends AirstrikeProjectile;
...
{
    local int i;
    local CBU100Projectile SmallShell;
    ...
    for (i=0; i<100; i++)
    {
        SmallShell = spawn(class'CBU100Projectile', self, , Location, Rotation);
		if ( SmallShell != None )
...
{
	if (Other != Instigator && !Other.IsA('CBU100BaseShell'))
	{
