...
simulated function PostBeginPlay()
{
	...
	{
		SmokeTrail = Spawn(class'NukeMissileTrail',self,,Location - 40 * Dir);
...
{
	if ( (Damage > 100) && ((InstigatedBy == None) || (InstigatedBy.Controller == None) || (Instigator == None) || (Instigator.Controller == None) || !InstigatedBy.Controller.SameTeamAs(Instigator.Controller)) )
	{
...
function BlowUp(vector HitLocation)
{
	...
	{
		bHidden = true;
        E = Spawn(class'CarpetBombExplosion',,, HitLocation - 100 * Normal(Velocity), Rot(0,16384,0));
		if ( Level.NetMode == NM_DedicatedServer )
		{
			E.LifeSpan = 0.7;
		}
...
simulated function DrawHUD(Canvas Canvas)
{
	...
	else if ( !Level.IsSoftwareRendering() )
	{
	    if (Canvas.ClipX >= Canvas.ClipY)
	    {
	        ...
	        Canvas.DrawTile( OuterScopeShader, 1.0 * Canvas.SizeX, 1.0 * Canvas.SizeY, 0, 0 * (1 - Offset), 512, 512 * Offset);
            Offset = Canvas.ClipY / Canvas.ClipX;
	       //Canvas.DrawTile( ColorModifier'GrayCM', 1.0 * Canvas.SizeX, 1.0 * Canvas.SizeY, 0, 0 * (1 - Offset), 512, 512 * Offset);
            Canvas.SetPos(0, 0);
	    ...
	    Canvas.DrawText( "THERMOBARIC WARHEAD MODE");  

        }
        else
        {
            Offset = Canvas.ClipY / Canvas.ClipX;
	        Canvas.DrawTile( Material'ScreenNoiseFB', 1.0 * Canvas.SizeX, 1.0 * Canvas.SizeY, 0, 0 * (1 - Offset), 512, 512 * Offset);
...
defaultproperties
{
     ...
     DrawScale=0.500000
     Skins(1)=TexEnvMap'CubeMaps.Core.DomCoreTexEnv'
     AmbientGlow=96
     bGameRelevant=True
     bCanTeleport=False
