...
// The X93IonCannon
//=============================================================================
class X93IonCannon extends Actor
    placeable;

var() Sound FireSound;
var() Vector MarkLocation;
var() class<X93IonEffect> IonEffectClass;
...
{
	local X93IonCannon C;
	
    Super.PostBeginPlay();
    if ( bDeleteMe )
		return;
		
	ForEach DynamicActors(Class'X93IonCannon',C)
	{
		if ( C != self )
			C.Destroy();
	}
...
state FireSequence
{
    ...
    {
        local X93IonEffect X93IonBeamEffect;
        local Actor Other;
        local Vector HitLocation, HitNormal, Top, CP;

        Other = Trace(HitLocation, HitNormal, MarkLocation + vect(0,0,1), MarkLocation, false);
 
		Top = MarkLocation;
		Top.Z = FMax(HitLocation.Z,Location.Z);

        X93IonBeamEffect = Spawn(IonEffectClass,,, Top);
        if (X93IonBeamEffect != None)
            X93IonBeamEffect.AimAt(MarkLocation, Vect(0,0,1));
        ...
        if ( Other == None )
        {
			X93IonBeamEffect = Spawn(IonEffectClass,,, Top + CP);
			if (X93IonBeamEffect != None)
				X93IonBeamEffect.AimAt(MarkLocation, Vect(0,0,1));
		}
    ...
    function EndState()
    {
		if ( (Instigator != None) && (X93Painter(Instigator.Weapon) != None) )
...
defaultproperties
{
     ...
     IonEffectClass=Class'BWBPAirstrikesPro.X93IonEffect'
     Damage=1500
     MomentumTransfer=150000.000000
     DamageRadius=7500.000000
