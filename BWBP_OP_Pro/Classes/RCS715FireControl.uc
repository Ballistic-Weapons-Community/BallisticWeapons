...
//=============================================================================
class RCS715FireControl extends Actor;
...
var array<RCS715GroundFire>	Fires;
...
simulated function Initialize()
{
	...
	{
		if (xPawn(A) != None && Level.Game != None && Level.Game.ReduceDamage(Damage, xPawn(A), Instigator, Location, Dummy, class'DT_RCS715Immolation') <= 0)
			continue;
		if (xPawn(A)!=None)
			IgniteActor(A);
	...
	for(i=0;i<MAX_FIRE_SPOTS;i++)
	{
		...
		GF = Spawn(class'RCS715GroundFire',self,,HitLoc, rot(0,0,0));
		if (GF!=None)
		{
...
simulated function PostNetReceive()
{
	...
	if (!bClientFiresSpawned)
	{
		for (i=0;i<ArrayCount(GroundFireSpots);i++)
		{
			...
			{
				F = Spawn(class'RCS715GroundFire',self,,GroundFireSpots[i], rot(0,0,0));
				if (F != None)
...
{
	if (RCS715GroundFire(A) == None)
		return;
	Fires[Fires.Length] = RCS715GroundFire(A);
}
//===========================================================================
...
{
	local RCS715ActorBurner PF;

	PF = Spawn(class'RCS715ActorBurner',self, ,A.Location);
