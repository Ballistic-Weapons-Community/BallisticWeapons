...
// G28CloudControl.
//
// An invisible actor responsible for handling a system of g28 gas clouds. It
...
//=============================================================================
class G28CloudControl_Exp extends Actor;

var   array<G28GasCloud_Exp>	Clouds;
var() int					    Damage;
...
simulated function CloudDie(G28GasCloud_Exp C)
{
	local int i;
...
{
	local G28GasCloud_Exp C;

	if (Owner != None)
		C = Spawn(class'G28GasCloud_Exp',self,,V, Rotator(Owner.Velocity));
	else if (Clouds.length>0 && Clouds[Clouds.length-1] != None)
		C = Spawn(class'G28GasCloud_Exp',self,,V, Rotator(V - Clouds[Clouds.length-1].Location));
	else if (Instigator != None)
		C = Spawn(class'G28GasCloud_Exp',self,,V, Rotator(V - Instigator.Location));
	else
		C = Spawn(class'G28GasCloud_Exp',self,,V);
	if (C != None)
	{
