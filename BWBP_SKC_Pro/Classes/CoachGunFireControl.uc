...
//=============================================================================
class CoachGunFireControl extends RX22AFireControl;
...
{
	local CoachGunFlameProjectile Proj;
	local int i;
	
	Proj = Spawn (class'CoachGunFlameProjectile',InstigatedBy,, Start, Dir);
	if (Proj != None)
	{
...
simulated function DoFlameHit(FlameHit Hit)
{
	...
	BurnRadius(4, 256, class'DT_CoachBurned', 0, Hit.HitLoc, Hit.Instigator);
...
{
	GasNodes[GasNodes.length] = Spawn(class'CoachGunSurfaceFire',InstigatedBy,,Loc, rotator(Norm));
...
{
	local CoachGunActorFire PF;

	PF = Spawn(class'CoachGunActorFire',InstigatedBy,,Other.Location, Other.Rotation);
	PF.SetFuel(Fuel);
	PF.Initialize(Other);
