...
//=============================================================================
class IM_BOGPFlareFizzle extends BCImpactManager;

simulated function SpawnEffects (int HitSurfaceType, vector Norm, optional byte Flags)
{
	local Projector P;
	local IE_BOGPFlareFizzle FizzleEffect;
	...
	if (HitEffects.Length > 0 && (Flags & HF_NoEffects) == 0)
	{
		...
		{
			FizzleEffect = IE_BOGPFlareFizzle(Spawn (HitEffects[0], Owner,, Location+EffectBackOff*Norm, Rotation));
			if(Pawn(Owner) != None)
		...
		{
			FizzleEffect = IE_BOGPFlareFizzle(Spawn (HitEffects[HitSurfaceType], Owner,, Location, Rotation));
			if(Pawn(Owner) != None)
				FizzleEffect.SetupColor(Pawn(Owner).GetTeamNum());
			else if(Controller(Owner) != None)
				FizzleEffect.SetupColor(Controller(Owner).GetTeamNum());
