...
// MRS138CloudControl.
//
// An invisible actor responsible for handling a system of MRS gas clouds. It
...
//=============================================================================
class MRS138CloudControl extends Actor;

var   array<MRS138Cloud>		Clouds;
var() int					Damage;
...
simulated function CloudDie(MRS138Cloud C)
{
	local int i;
...
simulated function SpawnCloud(vector V)
{
	...
	if (Owner != None)
		C = Spawn(class'MRS138Cloud',self,,V, Rotator(Owner.Velocity));
	else if (Clouds.length>0 && Clouds[Clouds.length-1] != None)
		C = Spawn(class'MRS138Cloud',self,,V, Rotator(V - Clouds[Clouds.length-1].Location));
	else if (Instigator != None)
		C = Spawn(class'MRS138Cloud',self,,V, Rotator(V - Instigator.Location));
	else
		C = Spawn(class'MRS138Cloud',self,,V);
	if (C != None)
	{
...
{
     Damage=4
     DamageType=Class'BallisticProV55.DTMRS138Gas'
     bHidden=True
