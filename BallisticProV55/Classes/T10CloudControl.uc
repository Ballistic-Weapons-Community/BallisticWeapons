...
//=============================================================================
class T10CloudControl extends Actor;

var   array<T10Cloud>		Clouds;
var() int					Damage;
...
simulated function CloudDie(T10Cloud C)
{
	local int i;
...
simulated function SpawnCloud(vector V)
{
	...
	if (Owner != None)
		C = Spawn(class'T10Cloud',self,,V, Rotator(Owner.Velocity));
	else if (Clouds.length>0 && Clouds[Clouds.length-1] != None)
		C = Spawn(class'T10Cloud',self,,V, Rotator(V - Clouds[Clouds.length-1].Location));
	else if (Instigator != None)
		C = Spawn(class'T10Cloud',self,,V, Rotator(V - Instigator.Location));
	else
		C = Spawn(class'T10Cloud',self,,V);
	if (C != None)
	{
...
{
     Damage=6
     DamageType=Class'BallisticProV55.DTT10Gas'
     bHidden=True
