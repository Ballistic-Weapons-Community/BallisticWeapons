...
//=============================================================================
class A42AcidProjectile extends BallisticProjectile;
...
{
	local A42HPReducer hpReducer;
	
	super.DoDamage(Other, HitLocation);
	
	if (Pawn(other) != None && Pawn(Other).Health > 0 && Vehicle(Other) == None)
	{
		hpReducer= A42HPReducer(Pawn(Other).FindInventoryType(class'A42HPReducer'));
	
		if (hpReducer == None)
		{
			Pawn(Other).CreateInventory("BallisticProV55.A42HPReducer");
			hpReducer = A42HPReducer(Pawn(Other).FindInventoryType(class'A42HPReducer'));
		}
...
defaultproperties
{
	 ...
	 MyDamageType=Class'BallisticProV55.DTA42AcidBlast'
     LightType=LT_Steady
