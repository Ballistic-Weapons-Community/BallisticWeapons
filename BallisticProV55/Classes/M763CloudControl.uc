...
// M763CloudControl.
//
// An invisible actor responsible for handling a system of M763 gas clouds. It
...
//=============================================================================
class M763CloudControl extends Actor;

var   array<M763Cloud>		Clouds;
var() int					Damage;
...
simulated function CloudDie(M763Cloud C)
{
	local int i;
...
simulated function SpawnCloud(vector V)
{
	...
	if (Owner != None)
		C = Spawn(class'M763Cloud',self,,V, Rotator(Owner.Velocity));
	else if (Clouds.length>0 && Clouds[Clouds.length-1] != None)
		C = Spawn(class'M763Cloud',self,,V, Rotator(V - Clouds[Clouds.length-1].Location));
	else if (Instigator != None)
		C = Spawn(class'M763Cloud',self,,V, Rotator(V - Instigator.Location));
	else
		C = Spawn(class'M763Cloud',self,,V);
	if (C != None)
	{
