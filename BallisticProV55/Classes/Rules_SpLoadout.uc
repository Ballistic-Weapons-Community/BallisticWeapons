...
function int NetDamage( int OriginalDamage, int Damage, pawn Injured, pawn instigatedBy, vector HitLocation, out vector Momentum, class<DamageType> DamageType )
{
	...
	if (Injured != None && instigatedBy != None && instigatedBy != Injured && instigatedBy.PlayerReplicationInfo != None)
		EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(instigatedBy.PlayerReplicationInfo)).PlayerDamage += Dmg;
...
Function ScoreKill(Controller Killer, Controller Killed)
{
	...
	if ((Killer == None || Killer == Killed) && Killed != None && Killed.PlayerReplicationInfo!= None && class<WeaponDamageType>(LastDT) != None)
	{
		...
		if ((LastDT.default.bSpecial || Killed.Pawn.Weapon.bSniping) && ClassIsChildOf(Killed.Pawn.Weapon.Class, class<WeaponDamageType>(LastDT).default.WeaponClass))
			EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killed.PlayerReplicationInfo)).SniperKills -= 1;
		else
			EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killed.PlayerReplicationInfo)).SniperKills -= 0.2;
			
		// Suicided by Hazard damage. This is pretty likely...
		if (BDT != None && BDT.static.IsDamage(",Hazard,"))
			EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killed.PlayerReplicationInfo)).HazardKills -= 1;
		else
			EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killed.PlayerReplicationInfo)).HazardKills -= 0.2;
		...
		if (BallisticWeapon(Killed.Pawn.Weapon) != None && BallisticWeapon(Killed.Pawn.Weapon).bWT_Shotgun && ClassIsChildOf(Killed.Pawn.Weapon.Class, class<WeaponDamageType>(LastDT).default.WeaponClass))
			EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killed.PlayerReplicationInfo)).ShotgunKills -= 1;
		else
			EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killed.PlayerReplicationInfo)).ShotgunKills -= 0.2;
	}
	// Reward for killing
	if (Killer != None && Killer.PlayerReplicationInfo!=None)
	{
		...
		if (LastDT.default.bSpecial)
			EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killer.PlayerReplicationInfo)).SniperKills += 1;
		...
		if (Dist > 1000 && (LastDT.default.bLocationalHit || LastDT.default.bInstantHit) && (BDT==None || !BDT.static.IsDamage(",NonSniper,")))
			EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killer.PlayerReplicationInfo)).SniperKills += (Dist-1000) / 8000;
		...
		{
			if (Dist <= 150)
				EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killer.PlayerReplicationInfo)).ShotgunKills += 1;
			if (Dist <= 1200)
				EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killer.PlayerReplicationInfo)).ShotgunKills += 1 - FMax(0, (Dist-150)/1050);
		}
		
		// Hazard skill reward for killing people with Hazardous weapons
		if (BDT != None && BDT.static.IsDamage(",Hazard,"))
			EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killer.PlayerReplicationInfo)).HazardKills += 1;
	}
	// Punish death
	...
	{	
		...
		if (Killed.Pawn.Weapon.bSniping)
			EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killed.PlayerReplicationInfo)).DeathsHoldingSniper += 1;
		if (BallisticWeapon(Killed.Pawn.Weapon) != None)
		{	// Holding a shotgun
			if (BallisticWeapon(Killed.Pawn.Weapon).bWT_Shotgun)
				EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killed.PlayerReplicationInfo)).DeathsHoldingShotgun += 1;
			else if (BallisticWeapon(Killed.Pawn.Weapon).bWT_Bullet && !BallisticWeapon(Killed.Pawn.Weapon).bSniping)
				EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killed.PlayerReplicationInfo)).DeathsHoldingShotgun+= 0.25;
			...
			if (BallisticWeapon(Killed.Pawn.Weapon).bWT_Hazardous)
				EliminationLRI(class'Mut_Ballistic'.static.GetBPRI(Killed.PlayerReplicationInfo)).DeathsHoldingHazard += 1;
		}
	}
}
